...
//!
//! # Contents
//! - A compact Navier-shaped dense-element loop whose accumulator joins a
//!   tagged initial element with numeric backedges.
...
//!   supplies a Float64 index, including integral boxing and exact fractional
//!   or negative-zero exits before indexed effects.
...
//!
//! # Invariants
//! - Empty-feedback numeric sites use guarded `DecodeNumber` operations rather
//!   than a cold-value exit or a generic element runtime transition.
...
//! - A Float64 element index is boxed at each indexed use. Integral int32-range
//!   values remain generated; fractional values and negative zero resume the
//!   canonical element operation without replaying an earlier effect.
...
fn boxed_number_element_byte_pc(optimized_ir: &str, opcode: &str) -> u64 {
    ...
    for pair in lines.windows(2) {
        if !pair[0].contains(" BoxNumber ") || !pair[1].contains(&opcode_marker) {
            continue;
        }
        let boxed = machine_values(pair[0]);
        let element = machine_values(pair[1]);
        if boxed.get(1) != element.get(1) {
            continue;
        }
        let byte_pc = pair[1]
            ...
            .unwrap_or_else(|| panic!("invalid {opcode} byte PC: {}", pair[1]));
        return byte_pc;
    }
    panic!("missing BoxNumber-fed {opcode} in Number-index Machine IR: {optimized_ir}")
}
...
fn assert_machine_navier_artifact(artifacts: &JitArtifactBatch) {
    ...
    for kind in ["machineElementLoad", "machineElementStore"] {
...
fn assert_machine_number_index_artifact(artifacts: &JitArtifactBatch) {
    ...
    for opcode in ["DecodeNumber", "FloatMul", "BoxNumber"] {
        assert!(
            optimized_ir.contains(opcode),
    ...
    let load_byte_pc = boxed_number_element_byte_pc(optimized_ir, "ElementLoad");
    let store_byte_pc = boxed_number_element_byte_pc(optimized_ir, "ElementStore");
    ...
    for (kind, byte_pc) in [
        ("machineElementLoad", load_byte_pc),
        ("machineElementStore", store_byte_pc),
    ] {
        assert!(
...
#[test]
fn guarded_number_element_indices_box_integral_and_exact_deopt_other_numbers() {
    ...
    assert_eq!(
        ...
        "integral Number index must box to the generated int32 path: {integral_delta:?}"
    );
    ...
    assert_eq!(
        negative_zero_delta.optimized_deopts, 1,
        "negative zero must retain its double box and exact-deopt once: {negative_zero_delta:?}"
    );
