...
fn construct_collection(
    ...
    // §24.1.1.1 step 8 — AddEntriesFromIterable. The adder lookup and
    // every entry insertion allocate, so the receiver and the source
    // iterable are parked in the handle scope: the scope keeps both live
...
pub(crate) fn is_map_prototype_builtin(method: Value, heap: &otter_gc::GcHeap, name: &str) -> bool {
    method
        .as_native_function()
        .zip(map_prototype_native(name))
        .is_some_and(|(native, target)| native.is_static_fn(heap, target))
}

/// Original `%Map.prototype%` native for `name` (get/set/has/delete).
pub(crate) fn map_prototype_native(name: &str) -> Option<crate::native_function::NativeFastFn> {
...
pub(crate) fn is_set_prototype_builtin(method: Value, heap: &otter_gc::GcHeap, name: &str) -> bool {
    method
        .as_native_function()
        .zip(set_prototype_native(name))
        .is_some_and(|(native, target)| native.is_static_fn(heap, target))
}

/// Original `%Set.prototype%` native for `name` (add/has/delete).
pub(crate) fn set_prototype_native(name: &str) -> Option<crate::native_function::NativeFastFn> {
    let target: crate::native_function::NativeFastFn = match name {
        "add" => set_proto_add,
        "has" => set_proto_has,
