...
#[derive(Parser, Debug)]
#[command(name = "test262-new")]
#[command(about = "Run Test262 conformance tests against the NEW Otter VM")]
struct Cli {
...
impl NewVmRunner {
    ...
    fn run_single_mode(
        ...
        let mut test_source = String::with_capacity(content.len() + 8192);
        ...
        if !metadata.is_raw() {
            ...
            for inc in &includes {
                if let Some(harness_content) = self.harness_cache.get(inc) {
        ...
        let test_body = content
            ...
            .map(|i| &content[i + 5..])
            .unwrap_or(content);
        test_source.push_str(test_body);
...
fn main() {
    ...
    eprintln!("{}", "Otter Test262 Runner (NEW VM)".bold().cyan());
