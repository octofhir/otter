...
**Phase 1 (Value layout & allocation reduction) DONE.** The 30-variant
`Value` enum has been replaced by `#[repr(transparent)] pub struct
Value(u64)` (NaN-boxed, `Copy`) at `crates/otter-vm/src/value/mod.rs:90`,
re-exported from `crates/otter-vm/src/lib.rs:190`. Pointer sub-tags
`TAG_PTR_OBJECT=0x7FFC`, `TAG_PTR_STRING=0x7FFD`,
`TAG_PTR_FUNCTION=0x7FFE`, `TAG_PTR_OTHER=0x7FFF` matched MEMORY.md.
Phases 1.1–1.5 (inventory, feature-flag, arith/load/store, property/call,
iter/async/generator) shipped sequentially; legacy enum and feature flag
deleted in the cut-over commit. `crates/otter-vm/src/legacy_value.rs`
removed. `is_object_type` (ECMA-262 §7.2.7 Type(value) is Object) is the
sole canonical predicate at `value/mod.rs:805`; legacy wrappers
(`is_callable_or_object_value`, `is_instance_object_value`,
`is_extended_object_value`, `is_object_like_value`,
`constructor_return_is_object`) deleted. `otter-vm --lib` 532/532
passing; `otter-runtime --lib` 123/123 passing; workspace `clippy` +
`fmt` clean. Boxed-Value migration of dependent layers (closures,
boxed-value rewrite of upvalue path) progressed through commits
`8825cf7b..ded0b4c5`.

**Remaining from Phase 1 follow-ups:**

- `runtime_array_cap_is_catchable_as_range_error` (
  `crates/otter-runtime/tests/runtime_oom_surfaces_as_error.rs`) currently
  `#[ignore]`. Root cause: cap-trigger `collect_full` in `otter-gc`
  visits only `handle_stack` + `global_handles` + local
  allocation-context roots; the interpreter's `error_classes` registry is
  traced from `runtime_state::trace_roots`, which only runs from
  `Interpreter::force_gc` (`lib.rs ~1992`). When the cap fires inside a
  script allocation, the `RangeError` constructor's `JsObject` is swept;
  `e instanceof RangeError` then dereferences a freed offset (type tag
  `0`). The right fix is an `extra_roots` callback on `GcHeap` invoked
  from `account_or_collect_with_roots` and `shade_roots`, plus moving
  `force_gc` to *use* that callback rather than duplicating the root walk
  — otherwise the trampoline re-fires while `RuntimeState::trace_roots`
  is already on the stack and breaks
  `gc_invariants::weak_refs_and_finalization::finalization_callback_cannot_observe_collected_target_through_weak_ref`
  (misaligned-pointer panic in the scavenger). Since `otter-vm` has
  workspace `forbid(unsafe_code)`, the trampoline either needs a local
  `[lints.rust]` override (matching `otter-gc`) or a per-item allow on a
  helper module.

**Phase 2/3/4 status:** unchanged from the plan below.
...
## Phase 1 — Value layout & allocation reduction (direction C) — DONE 2026-05-23

> The single biggest perf lever. Audit §1 already named it.
> This phase plans how to land it without rewriting the world
> twice.
>
> **Status (2026-05-23):** shipped. `Value(u64)` NaN-box live at
> `crates/otter-vm/src/value/mod.rs:90`; tag layout matches §1.4 / §1.8
> recommendation (LuaJIT-style sub-tags
> `TAG_PTR_OBJECT=0x7FFC`/`STRING=0x7FFD`/`FUNCTION=0x7FFE`/`OTHER=0x7FFF`).
> Phases 1.1–1.5 landed sequentially; parallel feature flag deleted at
> cut-over. 532/532 lib + 123/123 runtime tests green; one `#[ignore]`
> test remains (`runtime_array_cap_is_catchable_as_range_error`) blocked
> on the GC-root extra-callback work described in §1.9 below. Boxed-value
> migration for upvalue path also done (commits `8825cf7b..ded0b4c5`).
...
| Decision | Recommendation | Resolution (2026-05-23) |
| --- | --- | --- |
| Tag-bit assignment: V8-style top-tag vs LuaJIT-style payload-tag | LuaJIT-style NaN-box (per MEMORY.md historical design). Closer to JSC, simpler than V8's compressed pointers, fits Otter's 4 GiB GC cage exactly | **LuaJIT-style adopted.** Sub-tags `0x7FFC..0x7FFF` live at `value/tag.rs` |
| `Value` ordering: SMI fast path vs unified double | Hybrid: `Number(Smi(i32))` payload = LSB-tagged into a 32-bit half of the NaN payload (the existing `NumberValue` Smi already exists; map it onto bit layout). `Number(Double)` = canonical NaN payload encoding | **Hybrid adopted.** SMI in low 32 bits of NaN payload |
| Migration mode | Feature-flag parallel build, single-PR cut-over | **Done.** Feature flag and old enum deleted in cut-over commit |

### 1.9 Phase 1 follow-up — GC extra-roots callback (open)

The cap-trigger path in `otter-gc` collects only `handle_stack` +
`global_handles` + local allocation-context roots when
`account_or_collect_with_roots` / `shade_roots` fire mid-allocation. The
interpreter's `error_classes` registry is traced exclusively from
`runtime_state::trace_roots`, which only runs via
`Interpreter::force_gc` (`crates/otter-vm/src/lib.rs ~1992`). When the
heap cap fires inside script allocation the `RangeError` constructor's
`JsObject` is swept; `e instanceof RangeError` then dereferences a freed
offset (`type_tag=0`). This is what `runtime_array_cap_is_catchable_as_range_error`
exercises, and it remains `#[ignore]` after the migration.

The correct fix is an `extra_roots` callback on `GcHeap` invoked from
both `account_or_collect_with_roots` and `shade_roots`. `Interpreter::force_gc`
must itself **consume** this callback rather than duplicating the root
walk, otherwise the trampoline re-fires while
`RuntimeState::trace_roots` is already on the stack and breaks
`gc_invariants::weak_refs_and_finalization::finalization_callback_cannot_observe_collected_target_through_weak_ref`
(misaligned-pointer panic in the scavenger). The trampoline needs
`unsafe` for the raw-pointer indirection; since `otter-vm` has workspace
`forbid(unsafe_code)`, either add a local `[lints.rust]` override
mirroring `otter-gc`, or move the helper into a per-item-allowed
sub-module.

**Effort:** S (under one day once the API shape is agreed). **Risk:**
Medium — the scavenger interaction is exactly the trap a naive
implementation hit on the first attempt.
