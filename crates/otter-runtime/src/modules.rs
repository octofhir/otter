...
fn module_path_from_url(module_url: &str) -> Option<String> {
    let url = Url::parse(module_url).ok()?;
    if url.scheme() != "file" {
        return None;
    }
    url.to_file_path()
...
fn module_dirname(module_url: &str) -> Option<String> {
    module_path_from_url(module_url).and_then(|path| {
        Path::new(&path)
            .parent()
            .and_then(|parent| parent.to_str().map(|s| s.to_string()))
    })
}

fn module_filename(module_url: &str) -> Option<String> {
    module_path_from_url(module_url)
}

pub fn import_meta_prefix(module_url: &str) -> String {
    let url_lit = serde_json::to_string(module_url).unwrap();
    let dirname_lit = module_dirname(module_url)
        .map(|d| serde_json::to_string(&d).unwrap())
        .unwrap_or_else(|| "undefined".to_string());
    let filename_lit = module_filename(module_url)
        .map(|f| serde_json::to_string(&f).unwrap())
        .unwrap_or_else(|| "undefined".to_string());
    format!(
        "const __otter_import_meta__ = {{ url: {}, dirname: {}, filename: {} }};\n",
        url_lit, dirname_lit, filename_lit
    )
}
...
pub fn transform_module(
    ...
    let transformed = match transform_module_ast(source, module_url, dependencies) {
        Ok(code) => code,
        Err(_) => {
            ...
            let import_namespace_re =
                Regex::new(r#"(?m)^(\s*)import\s+\*\s+as\s+(\w+)\s+from\s+['"]([^'"]+)['"];\s*$"#)
                    .unwrap();

            let import_side_effect_re =
                Regex::new(r#"(?m)^(\s*)import\s+['"]([^'"]+)['"];\s*$"#).unwrap();

            let import_default_named_re = Regex::new(
                r#"(?m)^(\s*)import\s+(\w+)\s*,\s*\{([^}]+)\}\s+from\s+['"]([^'"]+)['"];\s*$"#,
            )
            .unwrap();
            ...
            result = import_default_named_re
            ...
            // Dynamic imports are transformed by the AST transformer in modules_ast.rs
            // during transform_module_ast() and transform_dynamic_imports() calls.
            ...
            let export_default_re =
                Regex::new(r#"(?m)^(\s*)export\s+default\s+(.+?);?\s*$"#).unwrap();

            let export_const_re =
                Regex::new(r#"(?m)^(\s*)export\s+(const|let|var)\s+(\w+)\s*="#).unwrap();
    ...
    let prefix = import_meta_prefix(module_url);
...
mod tests {
    ...
    fn test_transform_import_default_and_named() {
        ...
        assert!(
            result.contains(r#"__otter_modules["file:///node_modules/react/index.js"].default"#)
        );
        assert!(
            result.contains(r#"__otter_modules["file:///node_modules/react/index.js"].useState"#)
        );
    }
    ...
    fn test_transform_import_meta() {
        let source = "console.log(import.meta.url, import.meta.dirname);";
        let deps = HashMap::new();

        let result = transform_module(source, "file:///project/main.js", &deps);
        assert!(
            result.starts_with("const __otter_import_meta__ = {"),
            "missing import.meta prefix: {}",
            result
        );
        assert!(result.contains("__otter_import_meta__.url"));
        assert!(result.contains("__otter_import_meta__.dirname"));
    }
    ...
    fn test_bundle_modules_mixed_cjs_only() {
        ...
        assert!(
            bundle.contains("Object.defineProperty(__otter_modules, \"file:///project/lib.cjs\"")
        );
