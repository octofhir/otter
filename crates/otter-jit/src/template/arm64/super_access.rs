//! `super` property access transition emission.
//!
//! # Contents
//! - Reentrant calls to the VM-owned super read/write helper.
//! - Uniform success, throw, and exact pre-effect bailout routing.
//!
//! # Invariants
//! - The VM helper commits every supported super opcode before returning
...
//!   any observable home-prototype accessor getter/setter.
//!
//! # See also
...
pub(super) fn emit_super_op(
    ...
    emit_load_u64(ops, 16, transitions.variadic_entry(abi::STUB_JIT_SUPER_OP));
    dynasm!(ops
        ; .arch aarch64
        ; blr x16
        ; cbz x0, =>done
