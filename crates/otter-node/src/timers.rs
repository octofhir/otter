...
pub fn timers_cjs_value(ctx: &mut NativeCtx<'_>, _caps: &CapabilitySet) -> Result<Value, String> {
    otter_runtime::run_builtin_cjs_shim(ctx, "node:timers", TIMERS_SHIM, &[])
}

/// CommonJS export: the `timers/promises` namespace.
pub fn timers_promises_cjs_value(
    ctx: &mut NativeCtx<'_>,
    _caps: &CapabilitySet,
) -> Result<Value, String> {
    otter_runtime::run_builtin_cjs_shim(ctx, "node:timers/promises", TIMERS_PROMISES_SHIM, &[])
}
