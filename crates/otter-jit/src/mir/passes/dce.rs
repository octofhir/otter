...
pub fn run(graph: &mut MirGraph) {
    ...
    for block in &mut graph.blocks {
        block
            .instrs
            .retain(|instr| live.contains(&instr.value) || has_side_effects(&instr.op));
    }
}
...
fn collect_operands(op: &MirOp, out: &mut impl Extend<ValueId>) {
    ...
    match op {
        ...
        | MirOp::GuardBool { val, .. }
        | MirOp::GuardNotHole { val, .. } => {
            vals.push(*val);
        }

        MirOp::GuardShape { obj, .. } | MirOp::GuardArrayDense { obj, .. } => {
            vals.push(*obj);
        }
        ...
        MirOp::ArrayLength(v) | MirOp::ArrayPush { arr: v, .. } => {
            vals.push(*v);
        }
        ...
        MirOp::Throw(v)
        | MirOp::GetIterator(v)
        | MirOp::IteratorNext(v)
        | MirOp::IteratorClose(v) => {
            vals.push(*v);
        }
        ...
        MirOp::Safepoint { live } => {
            vals.extend_from_slice(live);
        }
        MirOp::WriteBarrier(v) | MirOp::Spread(v) => {
            vals.push(*v);
        }
