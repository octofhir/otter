...
- 2026-04-16: **M1 landed** — `ModuleCompiler` now lowers `function f(n) { return n + 1 }` end-to-end. Accepts a single top-level `FunctionDeclaration` with 0–1 simple params and a body = one `ReturnStatement`; return expressions can be an identifier (parameter ref), an int32-safe `NumericLiteral`, or a `+` of two int32-safe operands (`AddSmi imm` for `i8`-fit RHS literals, `Add reg` for identifier RHS). Everything else → `SourceLoweringError::Unsupported { construct }` with a stable tag. Emitted shape for `f(n) { return n + 1 }` is `Ldar r0 / AddSmi 1 / Return`. 20 new `source_compiler` unit tests covering supported + every negative path. `cargo build --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --all --check`, `cargo test --workspace` all green.
- 2026-04-16: **M2 landed** — added `m2_stencil_disassembly_sanity` (compiles `function f(n) { return n + 1 }` via `ModuleCompiler`, runs the v2 baseline analyzer + emitter, disassembles the resulting stencil with `bad64`, asserts the expected mnemonics — `LDR / EOR / TST / B_NE / SXTW / MOV / ADD / ORR / RET` — appear and the byte size sits ≤ 200 B; current ≈160 B) and `m1_microbench` (`#[ignore]`d release-only bench — 2000-call interpreter warmup + 1 M timed `Interpreter::execute_with_runtime` calls; reports `interp: 496 ns/iter` on Apple Silicon for the M1 lowering). JIT half deferred: `execute_with_runtime` is a top-level entry, so the JSC tier-up hook only fires on inner `CallClosure` boundaries (not until M9), and direct stencil invocation still hits the documented macOS `MAP_JIT` hazard. Quality gate green (`cargo build --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --all --check`, `cargo test --workspace`).
- 2026-04-16: **M3 landed** — `lower_binary_expression` now table-drives over a `binary_op_encoding` map covering `+/-/*/|/&/^/<</>>/>>>`. Operators with a Smi opcode (`+`, `-`, `*`, `|`, `&`, `<<`, `>>`) take the `*Smi imm` fast path on `i8`-fit literal RHS; `^` and `>>>` (no Smi opcode) accept identifier RHS only — literal RHS reuses the existing `wide_integer_literal_on_rhs` rejection until M4 lands a scratch slot. 16 new `source_compiler` unit tests (Reg + Smi for each operator with a Smi opcode, Reg-only for `^`/`>>>`, plus literal-RHS negatives for `^`/`>>>`/wide-`SubSmi`); the obsolete `non_addition_binary_unsupported` test became `comparison_binary_unsupported` and `division_unsupported_at_m3`. Quality gate green; the Module integration with the v2 dispatcher needed no changes — every M3 opcode was already covered by `interpreter/dispatch.rs`.
- 2026-04-17: **M9 landed** — multiple top-level `FunctionDeclaration`s plus `CallExpression` to any named top-level function. Entry = last-declared function (conventional `main` pattern). Names collected in a first pass so forward references (including mutual recursion like `is_even` / `is_odd`) resolve; a second pass lowers each body with the shared name table available. New `lower_call_expression` acquires a contiguous temp-slot window via `LoweringContext::acquire_temps` (backed by `Cell<RegisterIndex>` so it can run under `&LoweringContext`), materialises each arg, emits `CallDirect(func_idx, RegList { base, count })`, and releases temps LIFO so nested calls (`f(g(1, 2), h(3))`) get distinct windows. `FrameLayout` now uses `peak_temp_count` as `temporary_count`. `apply_binary_op_with_acc_lhs` got a complex-RHS fallback for call / nested-binary / parenthesised / assignment RHS: commutative ops spill LHS to one temp and use `Op r_lhs_temp`; non-commutative ops grab a second temp for RHS and re-stitch via `Ldar r_lhs_temp; Op r_rhs_temp`. This is what lets `n * fact(n - 1)` compile. `CallExpression` wired into both `lower_return_expression` (any acc-producing position) and the `ExpressionStatement` arm (result discarded). `run_int32_function` test helper switched to `module.entry()` since the entry is no longer always `FunctionIndex(0)`. **Tier-up path lit up**: `CallDirect` internally routes through `run_with_tier_up`, so inner functions called from outer bodies accrue the JSC hotness budget even though the outer entry itself doesn't tier up. 16 new `source_compiler` unit tests covering zero/one/multi-arg calls, mutual recursion, factorial, call-in-loop, call-as-statement, plus negatives for unbound function, member callee, spread arg, duplicate top-level function, calling a parameter, and `new` expression. otter-vm 355 → 371.
- 2026-04-17: **M8 landed** — `ForStatement` (`for (init; test; update) body`), desugared to the standard `<init>; loop_header: <test or LdaTrue>; JumpIfToBooleanFalse exit; <body>; <update>; Jump loop_header; exit:` shape. Init accepts a `let`/`const` declaration (single or multi-declarator) or an `AssignmentExpression`; missing init/test/update each have their own well-defined fallthrough (omitted test ⇒ `LdaTrue`, omitted init/update ⇒ skip). New `LoweringContext::snapshot_scope` / `restore_scope` plumbing scopes for-init `let i = 0` to the loop while a new `peak_local_count` field retains the FrameLayout reservation so popped slots stay reserved for the function lifetime. Because `for` may introduce bindings inside nested contexts, `lower_nested_statement`, `lower_if_statement`, and `lower_while_statement` were retyped from `&LoweringContext` → `&mut LoweringContext` (the expression-lowering helpers stay `&` since they only read). `for (… in …)` and `for (… of …)` are separate AST node types and stay rejected. 15 new `source_compiler` unit tests covering classic for-init-`let`, for-init-assignment, omitted init/test/update, compound-assign update, multi-declarator init, for-inside-if, const-init reassignment rejection, plus negatives for `for-in`/`for-of`/bare init/bare update — and a documented `nested_for_with_same_name_unsupported_until_block_scoping` placeholder for cross-scope shadowing (lands when block scoping does). otter-vm 340 → 355. v2 dispatcher needed no changes — `ForStatement` reuses the same Jump / JumpIfToBooleanFalse / LdaTrue opcode set.
- 2026-04-17: **M7 landed** — `WhileStatement` plus the multi-declarator `let s = 0, i = 0;` shape and the `(s + i) | 0` parenthesised-binary LHS idiom. Closes the bench2.ts surface from `V2_MIGRATION_PLAN.md`. New `lower_while_statement` emits the standard `loop_header: <test>; JumpIfToBooleanFalse loop_exit; <body>; Jump loop_header; loop_exit:` shape — the trailing backward jump decrements the JSC tier-up budget exactly the way the JIT expects on every iteration. `lower_let_const_declaration` now iterates declarators left-to-right (each gets its own slot via the existing `lower_single_declarator`) so multi-declarator `let` works without churning the M4 single-declarator path. `lower_accumulator_operand` is now a thin alias for `lower_return_expression`, which lifts the M3 restriction that the LHS of a binary expression couldn't itself be a binary or assignment — that's what unlocks `(s + i) | 0` (parenthesised binary LHS lowers cleanly into acc, outer `| 0` then operates against it). Body grammar wires `Statement::WhileStatement` into `lower_nested_statement` so loops are accepted inside `if` branches and inside other loops; `let`/`const` inside a loop body still rejects as `nested_variable_declaration`. 11 new `source_compiler` unit tests covering the bench2 sum-loop (n ∈ {0, 1, 10, 100}), nested while loops, while-inside-if, early-return-in-while, plus negatives for nested let, `break`, `continue`, `do-while`, `for`. The obsolete `multiple_declarators_unsupported` test was rewritten as `multiple_declarators_compose`. otter-vm 329 → 340. Added `bench2_microbench` (`#[ignore]`d, release-only) under `crates/otter-jit/src/baseline/mod.rs::tests` — measures `sum(1_000_000)` through the v2 interpreter on a persistent `RuntimeState`: **416 ns/inner-iter on Apple Silicon (≈ 416 ms/call)**. JIT half remains deferred (top-level `execute_with_runtime` doesn't trigger tier-up; bench2 needs the M9 call-expression path or the still-broken direct-stencil-invocation route on macOS). v2 dispatcher needed no changes.
- 2026-04-16: **M6 landed** — `IfStatement` plus the int32 relational operators `<`, `>`, `<=`, `>=`, `===`, `!==`. The body grammar is restructured so the function's last statement must be a `ReturnStatement`; earlier statements go through `lower_top_statement` (which accepts top-level `let`/`const` plus everything `lower_nested_statement` accepts), and `if` branches go through `lower_nested_statement` (no `let`/`const`, since M6 has no block scoping). New `lower_if_statement` emits the standard `<test>; JumpIfToBooleanFalse else_label; <consequent>; Jump end_label; else_label: <alternate>; end_label:` shape via `BytecodeBuilder::new_label`/`bind_label`/`emit_jump_to`. Relational ops live in a new `RelationalOpEncoding` table separate from `binary_op_encoding`; the lowering picks a forward (`acc <op> reg`) or swapped (`<op-swapped>` after loading the literal into acc) direction based on operand kinds, so `n < 5` lowers as `LdaSmi 5; TestGreaterThan r_n` ≡ `5 > n` without needing a scratch slot. `!==` is `TestEqualStrict; LogicalNot`. Two-literal comparisons (`5 < 10`) reject as `relational_needs_register_operand` until scratch slots arrive. The shared `emit_identifier_as_reg_operand` helper handles the TDZ check + error-tag plumbing for both arithmetic Reg-RHS and relational comparand paths. `run_int32_function` test helper switched to always use `execute_with_runtime` — the previous "try `execute()` first, fall back on error" path silently passed wrong-arm if-statement results once relational ops stopped throwing on undefined parameters. 19 new `source_compiler` unit tests covering each relational op (with and without operand swap), `if`/`if-else`/`else if` chains, mixed body grammars, plus negatives for nested let, two-literal comparison, loose `!=`, and the still-required trailing return after exhaustive if-else; otter-vm 311 → 329. v2 dispatcher needed no changes.
- 2026-04-16: **M5 landed** — `AssignmentExpression` (`=`, `+=`, `-=`, `*=`, `|=`) onto a local `let`. New `lower_assignment_expression` resolves the LHS to a writable local slot (rejecting const → `const_assignment`, parameter → `assignment_to_param`, member/destructuring/TS targets, in-TDZ binding → `tdz_self_reference`, undeclared identifier → `unbound_identifier`), then either lowers the RHS directly (plain `=`) or emits `Ldar r_target; <op> rhs` (compound) before persisting via `Star r_target`. The accumulator is left holding the assigned value so nested forms (`let y = x = 5;`, `return x = 5;`) compose without extra ldar/star round-trips. `BindingRef::Local` and `LocalBinding` gain an `is_const` flag captured at allocation time. `lower_binary_expression`'s RHS dispatch was extracted into `apply_binary_op_with_acc_lhs` so the compound-assignment path can reuse it. Body grammar widened to admit `ExpressionStatement` (only AssignmentExpression at statement position; bare reads/calls still rejected); compound operators outside `=`/`+=`/`-=`/`*=`/`|=` (`/=`, `%=`, `**=`, `<<=`, `>>=`, `>>>=`, `&=`, `^=`, `||=`, `&&=`, `??=`) surface stable per-operator tags via `assignment_operator_tag`. 21 new `source_compiler` unit tests (10 positives covering each supported op + value-flow patterns; 11 negatives covering const, param, undeclared, member/destructuring/TS targets, four unsupported compound ops, bare expression statement); otter-vm 290 → 311. v2 dispatcher needed no changes — `Star r_slot` is identical to the M4 init path.
- 2026-04-16: **M4 landed** — `let`/`const` with initializer. Body grammar widened from "exactly one return" to "zero or more `let`/`const` declarations followed by exactly one return"; FrameLayout grew a `local_count` slot and `lower_function_declaration` now lowers the body first to learn the count. New `LoweringContext` tracks per-function `LocalBinding { name, slot, initialized }`, with `allocate_local` rejecting parameter/local name collisions as `duplicate_binding` and resolving identifiers through a new `BindingRef::{Param, Local{initialized}}` enum. **Compile-time TDZ**: a binding becomes readable only after its post-init `Star r_slot`, so `let x = x + 1` (and the `let x = 1 + x` Reg-RHS path) surfaces as `Unsupported { construct: "tdz_self_reference" }` rather than executing — sufficient until M5/M6 introduce control flow. Negatives covered: `var`, `using`, bare `let x`, multi-declarator `let a = 1, b = 2`, destructuring binding, dead code after `return`, missing `return`, fractional initializer. 16 new `source_compiler` unit tests; otter-vm 274 → 290. Quality gate green; v2 dispatcher needed no changes (every `Star r_slot` / `Ldar r_slot` opcode was already covered).
