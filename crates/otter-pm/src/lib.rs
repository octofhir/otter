...
//! metadata/cache and tarball-cache foundations, and traits for
//! resolver/cache/install backends. Network registry fetching, tarball
//! extraction, and install mutation are still deferred.
//!
//! # Contents
...
impl PackageGraph {
    ...
    pub fn insert_bin(&mut self, bin: PackageBin) {
        self.bins.entry(bin.name.clone()).or_default().push(bin);
...
pub async fn enrich_resolution_with_registry_metadata(
    ...
    let registry_ids = resolution
        ...
        .collect::<Vec<_>>();
    for (id, name, range) in registry_ids {
        ...
        for (dep_name, dep_range) in &version.dependencies {
            package
                ...
                .or_insert_with(|| PackageId::registry(dep_name, dep_range).to_string());
        }

        let graph_id = PackageId::new(id);
...
pub async fn cache_registry_tarballs(
    ...
    let mut sources = lockfile
        .packages
        .values()
        .filter_map(|package| match &package.resolved {
            Some(ResolvedSource {
                kind: ResolvedSourceKind::Registry,
                reference,
            }) if reference.ends_with(".tgz") || reference.starts_with("http") => {
                Some(TarballSource {
                    url: reference.clone(),
...
fn binary_name_from_package_name(name: &str) -> &str {
    name.rsplit('/').next().unwrap_or(name)
}

fn package_name_path(name: &str) -> PathBuf {
