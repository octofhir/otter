...
mod tests {
    ...
    #[test]
    fn test_incremental_gc_traces_chain() {
        let registry = AllocationRegistry::new();

        // Create chain: root -> node1 -> node2
        let node2 = unsafe {
            gc_alloc_in(
                &registry,
                Node {
        ...
        // Incremental GC: process 1 object per step to test incremental behavior
        registry.start_incremental_gc(&[node1_header]);

        // Step 1: process node1 (discovers node2)
        let done = registry.incremental_mark_step(1);
        assert!(!done); // node2 still in worklist

        // Step 2: process node2
        let done = registry.incremental_mark_step(1);
        assert!(done); // worklist empty
    ...
    #[test]
    fn test_incremental_gc_budgeted() {
        let registry = AllocationRegistry::new();

        // Create chain of 10 nodes
        let mut headers = Vec::new();
        let mut prev_header: Option<*const GcHeader> = None;
        ...
        for i in 0..10 {
            ...
            let header = unsafe {
                (node as *const u8).sub(std::mem::size_of::<GcHeader>()) as *const GcHeader
            };
            headers.push(header);
            prev_header = Some(header);
        }

        let root = *headers.last().unwrap();

        registry.start_incremental_gc(&[root]);

        // Process with budget of 3 at a time
        let mut steps = 0;
        loop {
            let done = registry.incremental_mark_step(3);
            steps += 1;
            if done {
                break;
            }
        ...
        // Should take multiple steps to process 10 nodes with budget of 3
        assert!(steps >= 3);

        let _reclaimed = registry.finish_gc();
        assert_eq!(registry.allocation_count(), 10);
    }

    #[test]
    fn test_black_allocation_during_marking() {
        let registry = AllocationRegistry::new();
        ...
        // Allocate during marking → should be pre-marked Black
        let ptr = unsafe { gc_alloc_in(&registry, 42i32) };
        let header_ptr =
            unsafe { (ptr as *const u8).sub(std::mem::size_of::<GcHeader>()) as *const GcHeader };

        unsafe {
            assert_eq!((*header_ptr).mark(), MarkColor::Black);
        }

        // Complete marking and sweep — black-allocated object survives
        let done = registry.incremental_mark_step(1000);
        assert!(done);
        let _reclaimed = registry.finish_gc();
