...
| M35      | ES module imports + exports.                                                                                                    | [x]    | 334869f |
...
| ES-module surface: static `import` (named / default / namespace / renamed), `export function` / `export default function` / `export { … }` / re-exports (`export { x } from`, `export *`, `export * as ns`), plus dynamic `import(expr)` + `import.meta`. Compiler synthesises a module-init `VmFunction` that `CreateClosure`s + `StaGlobal`s each top-level export before `capture_exports` reads the namespace from the global object. `lower_identifier_reference` / `lower_direct_call` consult a per-module `module_globals` Rc-threaded list so a nested function body resolves an imported binding via `LdaGlobal`. `DynamicImport` dispatch delegates to `module_loader::dynamic_import_resolve` which walks the graph + evaluates missing modules on demand and returns a fulfilled `Promise` of the namespace, with the host + registry handed in via `execute_module_graph_shared` (`Rc<dyn ModuleHost>` + `Rc<RefCell<ModuleRegistry>>`) stashed in a thread-local for the duration of evaluation — no `unsafe` needed against the crate's `#![forbid(unsafe_code)]`. `import.meta.url` falls out of the same referrer channel | yes | M35 |
