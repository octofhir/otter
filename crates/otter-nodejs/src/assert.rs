...
mod tests {
    ...
    #[test]
    fn assert_module_is_callable_and_shared_across_aliases() {
        let dir = temp_test_dir("assert-cjs");
        std::fs::write(
            dir.join("main.cjs"),
            "const assertA = require('assert'); const assertB = require('node:assert'); const strict = require('assert/strict'); module.exports = [typeof assertA, assertA === assertB, typeof strict, assertA.strict === strict].join(':');",
        )
    ...
    #[test]
    fn assert_module_supports_core_node_style_checks() {
        let dir = temp_test_dir("assert-runtime");
        std::fs::write(
            dir.join("main.cjs"),
            "const assert = require('assert'); const strict = require('assert/strict'); assert(true); assert.strictEqual(NaN, NaN); assert.deepStrictEqual({ a: [1, 2] }, { a: [1, 2] }); assert.match('otter', /^ott/); assert.throws(() => { throw new TypeError('boom'); }, TypeError); assert.doesNotThrow(() => 1 + 1); strict.throws(() => strict.equal(1, '1')); globalThis.__assert_ok = 'yes';",
        )
        .expect("script should write");
