//! Ignition-style accumulator bytecode — the sole ISA of OtterJS.
//!
//! The full design lives in [`docs/bytecode-v2.md`](../../../../docs/bytecode-v2.md).
//! This module owns the self-contained ISA library: opcodes, encoder,
//! decoder, and the PC→feedback-slot side table. It does **not** hold
//! AST lowering, interpreter, or JIT logic — those live in
//! [`crate::source_compiler`], [`crate::interpreter::dispatch`], and
//! [`otter_jit::baseline`] respectively.
//!
//! ## Contract for callers
//!
//! The encoder ([`BytecodeBuilder`]) and the decoder ([`InstructionIter`])
//! together guarantee:
//!
...
pub use opcodes::{Opcode, OperandShape};
pub use operand::{Operand, OperandKind, OperandWidth};

/// Program-counter type: byte offset into [`Bytecode::bytes`].
///
/// Kept as a bare `u32` alias so VM components (activations, stack frames,
/// source maps, exception tables) can carry a PC without pulling in the
/// whole ISA module.
...
/// templates, deopt descriptors, etc. are owned by the `Function` metadata.
