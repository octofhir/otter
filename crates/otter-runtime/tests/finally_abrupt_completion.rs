...
fn return_runs_finally() {
    assert_eq!(
        run("var f = 0; function g(){ try { return 1; } finally { f = 9; } } var r = g(); r + ',' + f;"),
        "1,9"
    );
}
...
fn finally_return_overrides() {
    assert_eq!(run("function g(){ try { return 1; } finally { return 2; } } String(g());"), "2");
}

#[test]
fn nested_finally_runs_inner_then_outer_on_return() {
    assert_eq!(
        run("var o=[]; function g(){ try { try { return 1; } finally { o.push('i'); } } finally { o.push('o'); } } var r=g(); r+':'+o.join(',');"),
        "1:i,o"
    );
}
...
fn labelled_break_runs_each_crossed_finally_inner_first() {
    assert_eq!(
        run("var o=[]; L: for (var i=0;i<2;i++){ try { try { break L; } finally { o.push('a'); } } finally { o.push('b'); } } o.join(',');"),
        "a,b"
    );
}
...
fn async_return_runs_finally() {
    ...
    assert_eq!(
        run("var f=0; var out='?'; async function g(){ try { return 5; } finally { f = 1; } } g().then(v => { out = v + ',' + f; }); out;"),
        "?"
    );
}
...
fn break_without_finally_is_unaffected() {
    assert_eq!(run("var n=0; for (var i=0;i<5;i++){ if (i===2) break; n++; } String(n);"), "2");
}
