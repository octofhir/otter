...
impl Interpreter {
    ...
    /// Park a `BigInt` built from a signed 64-bit integer. Allocates a BigInt
    /// body, so — unlike [`Self::scoped_number`] — this preserves the full
    /// 64-bit range. Used by hosts that surface integral 64-bit values (e.g.
    /// WebAssembly `i64`) as JS `BigInt` rather than a lossy `Number`.
    pub(crate) fn scoped_bigint_i64<'s>(
        &mut self,
        scope: &'s HandleScope,
        n: i64,
    ) -> Result<Local<'s>, VmError> {
...
impl Interpreter {
    ...
    /// set — the raw heap collection without the VM's weak-ref/ephemeron
    /// processing, so it exercises rooting alone. Tests only; a production
    /// full collection is [`crate::Interpreter::force_gc`].
...
mod tests {
    ...
    fn scoped_wide_number_write_keeps_sibling_handle_live() {
        // On the host-side path (no dispatch provider), a collection during a
        ...
        let (moved, sibling_content, target_number) = interp.with_handle_scope(|interp, s| {
            ...
            // Store a wide double through the direct-Value slot path.
            ...
            // Force a minor collection that explicitly roots only the receiver.
            // The sibling survives through the scope-registered runtime-root
            // provider (the arena). Churn young space so the scavenge has a
            // survivor to relocate.
            let mut after = before;
            let mut moved = false;
            for _ in 0..8 {
                let _ = interp.scoped_object(s).unwrap();
