...
use oxc_ast::ast::{NumericLiteral, StringLiteral, RegExpLiteral, TemplateLiteral};
...
impl LiteralValidator {
    ...
    pub fn validate_numeric_literal(&self, lit: &NumericLiteral) -> CompileResult<()> {
        ...
        if self.strict_mode && self.is_legacy_octal(raw) {
            return Err(CompileError::legacy_syntax(
                format!("Legacy octal literal '{}' is not allowed in strict mode", raw),
                lit.span.start,
                lit.span.start + 1, // Approximate column
            ));
        }
    ...
    fn is_legacy_octal(&self, raw: &str) -> bool {
        ...
        // Not legacy octal if it's hex, binary, or decimal
        ...
        // Check if all remaining characters are octal digits (0-7)
    ...
    fn is_valid_numeric_separator_usage(&self, raw: &str) -> bool {
        ...
        if raw.starts_with("0x_") || raw.starts_with("0X_") ||
           raw.starts_with("0b_") || raw.starts_with("0B_") ||
           raw.starts_with("0o_") || raw.starts_with("0O_") {
            return false;
        }
        ...
        // Cannot have separators around exponent
        if raw.contains("_e") || raw.contains("_E") || 
           raw.contains("e_") || raw.contains("E_") {
            return false;
        }
                        ...
                        match next_ch {
                            ...
                            'x' => {
                                if !self.validate_hex_escape_sequence(&mut chars) {
                                    return Err(CompileError::invalid_literal(
                                        format!("Invalid hexadecimal escape sequence at position {}", position),
                            ...
                            '1'..='7' => {
                                if self.strict_mode {
                                    return Err(CompileError::legacy_syntax(
                                        format!("Legacy octal escape sequence '\\{}' is not allowed in strict mode", next_ch),
                                        start_pos + position as u32,
                                        start_pos + position as u32 + 1,
                                    ));
                                }
                            ...
                            '8' | '9' => {
                                if self.strict_mode {
                                    return Err(CompileError::legacy_syntax(
                                        format!("Legacy numeric escape sequence '\\{}' is not allowed in strict mode", next_ch),
                                        start_pos + position as u32,
                                        start_pos + position as u32 + 1,
                                    ));
                                }
                            ...
                            _ => {
                                ...
                                if self.strict_mode && !self.is_valid_escape_character(next_ch) {
                                    return Err(CompileError::invalid_literal(
                                        format!("Invalid escape sequence '\\{}' in strict mode", next_ch),
                                        start_pos + position as u32,
                                        start_pos + position as u32 + 1,
                                    ));
                                }
    ...
    fn validate_hex_escape_sequence(&self, chars: &mut std::iter::Peekable<std::str::Chars>) -> bool {
    ...
    fn validate_unicode_escape_sequence(&self, chars: &mut std::iter::Peekable<std::str::Chars>) -> CompileResult<usize> {
        match chars.peek() {
            Some('{') => {
                // \u{...} format
                chars.next(); // consume '{'
                ...
                while let Some(&ch) = chars.peek() {
                    if ch == '}' {
                        ...
                        if hex_digits == 0 || hex_digits > 6 {
                            return Err(CompileError::invalid_literal(
                                "Invalid Unicode escape sequence: must have 1-6 hex digits".to_string(),
                                0, 0, // Position will be adjusted by caller
                            ));
                        }
                        return Ok(consumed);
            ...
            _ => {
                ...
                for i in 0..4 {
                    match chars.peek() {
                        ...
                        _ => {
                            return Err(CompileError::invalid_literal(
                                format!("Invalid Unicode escape sequence: expected 4 hex digits, got {}", i),
                                0, 0, // Position will be adjusted by caller
                            ));
                        }
                    }
                    ...
                    Some(&next_ch) => {
                        ...
                        match next_ch {
                            ...
                            'x' => {
                                if !self.validate_hex_escape_sequence(&mut chars) {
                                    return Err(CompileError::invalid_literal(
                                        format!("Invalid hexadecimal escape sequence in RegExp at position {}", position),
                                        start_pos + position as u32,
                                        start_pos + position as u32 + 1,
                                    ));
                                }
    ...
    fn validate_regexp_character_classes(&self, pattern: &str, start_pos: u32) -> CompileResult<()> {
        let mut chars = pattern.chars().peekable();
        let mut _position = 0;
            ...
            match ch {
                ...
                ')' => {
                    if paren_depth > 0 {
                        ...
                        return Err(CompileError::invalid_literal(
                            format!("Unmatched closing parenthesis in RegExp at position {}", position),
                            start_pos + position as u32,
                            start_pos + position as u32 + 1,
                        ));
                    }
    ...
    fn validate_template_escape_sequences(&self, content: &str, start_pos: u32, quasi_index: usize) -> CompileResult<()> {
        let mut chars = content.chars().peekable();
        let mut position = 0;
        ...
        while let Some(ch) = chars.next() {
            if ch == '\\' {
                match chars.peek() {
                    Some(&next_ch) => {
                        ...
                        match next_ch {
                            ...
                            'x' => {
                                if !self.validate_hex_escape_sequence(&mut chars) {
                                    return Err(CompileError::invalid_literal(
                                        format!("Invalid hexadecimal escape sequence in template literal quasi {} at position {}", quasi_index, position),
                                        start_pos + position as u32,
                                        start_pos + position as u32 + 1,
                                    ));
                                }
                            ...
                            '1'..='7' => {
                                if self.strict_mode {
                                    return Err(CompileError::legacy_syntax(
                                        format!("Legacy octal escape sequence '\\{}' is not allowed in strict mode template literal", next_ch),
                                        start_pos + position as u32,
                                        start_pos + position as u32 + 1,
                                    ));
                                }
                            ...
                            '8' | '9' => {
                                if self.strict_mode {
                                    return Err(CompileError::legacy_syntax(
                                        format!("Legacy numeric escape sequence '\\{}' is not allowed in strict mode template literal", next_ch),
                                        start_pos + position as u32,
                                        start_pos + position as u32 + 1,
                                    ));
                                }
                            ...
                            _ => {
                                ...
                                if self.strict_mode && !self.is_valid_template_escape_character(next_ch) {
                                    return Err(CompileError::invalid_literal(
                                        format!("Invalid escape sequence '\\{}' in strict mode template literal", next_ch),
                                        start_pos + position as u32,
                                        start_pos + position as u32 + 1,
                                    ));
                                }
                    ...
                    None => {
                        return Err(CompileError::invalid_literal(
                            format!("Unterminated escape sequence in template literal quasi {}", quasi_index),
                            start_pos + position as u32,
                            start_pos + position as u32 + 1,
                        ));
                    }
...
mod tests {
    ...
    fn test_string_escape_sequence_validation() {
        ...
        assert!(validator.validate_string_escape_sequences("Hello\\nWorld", 0).is_ok());
        assert!(validator.validate_string_escape_sequences("Tab\\tSeparated", 0).is_ok());
        assert!(validator.validate_string_escape_sequences("Quote\\\"Mark", 0).is_ok());
        assert!(validator.validate_string_escape_sequences("Backslash\\\\", 0).is_ok());
        
        // Hexadecimal escapes
        assert!(validator.validate_string_escape_sequences("Hex\\x41", 0).is_ok());
        assert!(validator.validate_string_escape_sequences("Hex\\xFF", 0).is_ok());
        
        // Unicode escapes
        assert!(validator.validate_string_escape_sequences("Unicode\\u0041", 0).is_ok());
        assert!(validator.validate_string_escape_sequences("Unicode\\u{41}", 0).is_ok());
        assert!(validator.validate_string_escape_sequences("Unicode\\u{1F600}", 0).is_ok());
        
        // Line continuation
        assert!(validator.validate_string_escape_sequences("Line\\\nContinuation", 0).is_ok());
        assert!(validator.validate_string_escape_sequences("Line\\\r\nContinuation", 0).is_ok());
    }
    ...
    fn test_legacy_escape_sequences_strict_mode() {
        ...
        assert!(strict_validator.validate_string_escape_sequences("Octal\\1", 0).is_err());
        assert!(strict_validator.validate_string_escape_sequences("Octal\\77", 0).is_err());
        assert!(strict_validator.validate_string_escape_sequences("Octal\\377", 0).is_err());
        
        // Legacy numeric escapes (8, 9) should fail in strict mode
        assert!(strict_validator.validate_string_escape_sequences("Invalid\\8", 0).is_err());
        assert!(strict_validator.validate_string_escape_sequences("Invalid\\9", 0).is_err());
        
        // But should be OK in non-strict mode
        assert!(non_strict_validator.validate_string_escape_sequences("Octal\\1", 0).is_ok());
        assert!(non_strict_validator.validate_string_escape_sequences("Invalid\\8", 0).is_ok());
    }
    ...
    fn test_invalid_escape_sequences() {
        ...
        // Invalid hex escapes
        assert!(validator.validate_string_escape_sequences("BadHex\\xGG", 0).is_err());
        assert!(validator.validate_string_escape_sequences("ShortHex\\xF", 0).is_err());
        
        // Invalid Unicode escapes
        assert!(validator.validate_string_escape_sequences("BadUnicode\\uGGGG", 0).is_err());
        assert!(validator.validate_string_escape_sequences("ShortUnicode\\u41", 0).is_err());
        assert!(validator.validate_string_escape_sequences("BadBraceUnicode\\u{GGGG}", 0).is_err());
        assert!(validator.validate_string_escape_sequences("EmptyBraceUnicode\\u{}", 0).is_err());
        assert!(validator.validate_string_escape_sequences("UnterminatedBrace\\u{41", 0).is_err());
        
        // Unterminated escape
        assert!(validator.validate_string_escape_sequences("Unterminated\\", 0).is_err());
    }
    ...
    fn test_unicode_escape_validation() {
        ...
        assert_eq!(validator.validate_unicode_escape_sequence(&mut chars).unwrap(), 4);
        
        let mut chars = "41".chars().peekable();
        assert!(validator.validate_unicode_escape_sequence(&mut chars).is_err());
        ...
        assert_eq!(validator.validate_unicode_escape_sequence(&mut chars).unwrap(), 7);
    ...
    fn test_regexp_escape_sequences() {
        ...
        assert!(validator.validate_regexp_escape_sequences("\\.", 0).is_ok());
        assert!(validator.validate_regexp_escape_sequences("\\*", 0).is_ok());
        assert!(validator.validate_regexp_escape_sequences("\\x41", 0).is_ok());
        assert!(validator.validate_regexp_escape_sequences("\\u0041", 0).is_ok());
        assert!(validator.validate_regexp_escape_sequences("\\u{41}", 0).is_ok());
        
        // Invalid escape sequences
        assert!(validator.validate_regexp_escape_sequences("\\xGG", 0).is_err());
        assert!(validator.validate_regexp_escape_sequences("\\uGGGG", 0).is_err());
    ...
    fn test_regexp_character_classes() {
        ...
        assert!(validator.validate_regexp_character_classes("[abc]", 0).is_ok());
        assert!(validator.validate_regexp_character_classes("[a-z]", 0).is_ok());
        assert!(validator.validate_regexp_character_classes("[^abc]", 0).is_ok());
        assert!(validator.validate_regexp_character_classes("[[abc]]", 0).is_ok()); // Nested
        assert!(validator.validate_regexp_character_classes("", 0).is_ok()); // No classes
        
        // Invalid character classes
        assert!(validator.validate_regexp_character_classes("[abc", 0).is_err()); // Unterminated
        assert!(validator.validate_regexp_character_classes("[[abc]", 0).is_err()); // Unterminated nested
    }
    ...
    fn test_template_escape_sequences() {
        ...
        assert!(validator.validate_template_escape_sequences("Hello\\nWorld", 0, 0).is_ok());
        assert!(validator.validate_template_escape_sequences("Tab\\tSeparated", 0, 0).is_ok());
        assert!(validator.validate_template_escape_sequences("Quote\\`Mark", 0, 0).is_ok());
        assert!(validator.validate_template_escape_sequences("Dollar\\$Sign", 0, 0).is_ok());
        assert!(validator.validate_template_escape_sequences("Backslash\\\\", 0, 0).is_ok());
        
        // Hexadecimal and Unicode escapes
        assert!(validator.validate_template_escape_sequences("Hex\\x41", 0, 0).is_ok());
        assert!(validator.validate_template_escape_sequences("Unicode\\u0041", 0, 0).is_ok());
        assert!(validator.validate_template_escape_sequences("Unicode\\u{41}", 0, 0).is_ok());
        
        // Line continuation
        assert!(validator.validate_template_escape_sequences("Line\\\nContinuation", 0, 0).is_ok());
        assert!(validator.validate_template_escape_sequences("Line\\\r\nContinuation", 0, 0).is_ok());
    }
    ...
    fn test_template_legacy_escapes_strict_mode() {
        ...
        // Legacy octal escapes should fail in strict mode
        assert!(strict_validator.validate_template_escape_sequences("Octal\\1", 0, 0).is_err());
        assert!(strict_validator.validate_template_escape_sequences("Octal\\77", 0, 0).is_err());
        
        // Legacy numeric escapes (8, 9) should fail in strict mode
        assert!(strict_validator.validate_template_escape_sequences("Invalid\\8", 0, 0).is_err());
        assert!(strict_validator.validate_template_escape_sequences("Invalid\\9", 0, 0).is_err());
        
        // But should be OK in non-strict mode
        assert!(non_strict_validator.validate_template_escape_sequences("Octal\\1", 0, 0).is_ok());
        assert!(non_strict_validator.validate_template_escape_sequences("Invalid\\8", 0, 0).is_ok());
    }
    ...
    fn test_template_invalid_escapes() {
        ...
        // Invalid hex escapes
        assert!(validator.validate_template_escape_sequences("BadHex\\xGG", 0, 0).is_err());
        assert!(validator.validate_template_escape_sequences("ShortHex\\xF", 0, 0).is_err());
        
        // Invalid Unicode escapes
        assert!(validator.validate_template_escape_sequences("BadUnicode\\uGGGG", 0, 0).is_err());
        assert!(validator.validate_template_escape_sequences("ShortUnicode\\u41", 0, 0).is_err());
        
        // Unterminated escape
        assert!(validator.validate_template_escape_sequences("Unterminated\\", 0, 0).is_err());
    }
