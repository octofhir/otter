//! Temporal.PlainYearMonth constructor and prototype intrinsics.
//!
//! §13.2 Temporal.PlainYearMonth
//! <https://tc39.es/proposal-temporal/#sec-temporal-plainyearmonth-objects>

use std::cmp::Ordering;

use crate::descriptors::{
    JsClassDescriptor, NativeBindingDescriptor, NativeBindingTarget, NativeFunctionDescriptor,
    VmNativeCallError,
};
use crate::object::ObjectHandle;
use crate::value::RegisterValue;

use super::duration::to_duration;
use super::helpers::{self, temporal_err, to_integer_or_zero, to_string_arg};
use super::payload::{TemporalPayload, construct_temporal, require_temporal_payload};

// ── Descriptor ──────────────────────────────────────────────────────

pub fn plain_year_month_class_descriptor() -> JsClassDescriptor {
    JsClassDescriptor::new("PlainYearMonth")
        .with_constructor(NativeFunctionDescriptor::constructor(
            "PlainYearMonth",
            2,
            pym_constructor,
        ))
        .with_binding(stat("from", 1, pym_from))
        .with_binding(stat("compare", 2, pym_compare))
        .with_binding(getter("calendarId", pym_calendar_id))
        .with_binding(getter("year", pym_year))
        .with_binding(getter("month", pym_month))
        .with_binding(getter("monthCode", pym_month_code))
        .with_binding(getter("daysInMonth", pym_days_in_month))
        .with_binding(getter("daysInYear", pym_days_in_year))
        .with_binding(getter("monthsInYear", pym_months_in_year))
        .with_binding(getter("inLeapYear", pym_in_leap_year))
        .with_binding(proto("add", 1, pym_add))
        .with_binding(proto("subtract", 1, pym_subtract))
        .with_binding(proto("equals", 1, pym_equals))
        .with_binding(proto("toString", 0, pym_to_string))
        .with_binding(proto("toJSON", 0, pym_to_json))
        .with_binding(proto("valueOf", 0, helpers::temporal_value_of))
}

type VmNativeFn = fn(
    &RegisterValue,
    &[RegisterValue],
    &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError>;

fn proto(name: &str, arity: u16, f: VmNativeFn) -> NativeBindingDescriptor {
    NativeBindingDescriptor::new(
        NativeBindingTarget::Prototype,
        NativeFunctionDescriptor::method(name, arity, f),
    )
}

fn getter(name: &str, f: VmNativeFn) -> NativeBindingDescriptor {
    NativeBindingDescriptor::new(
        NativeBindingTarget::Prototype,
        NativeFunctionDescriptor::getter(name, f),
    )
}

fn stat(name: &str, arity: u16, f: VmNativeFn) -> NativeBindingDescriptor {
    NativeBindingDescriptor::new(
        NativeBindingTarget::Constructor,
        NativeFunctionDescriptor::method(name, arity, f),
    )
}

// ── Helpers ─────────────────────────────────────────────────────────

fn require_plain_year_month(
    this: &RegisterValue,
    runtime: &crate::interpreter::RuntimeState,
) -> Result<temporal_rs::PlainYearMonth, VmNativeCallError> {
    let payload = require_temporal_payload(this, runtime)
        .map_err(|_| VmNativeCallError::Internal("expected Temporal.PlainYearMonth".into()))?;
    payload
        .as_plain_year_month()
        .cloned()
        .ok_or_else(|| VmNativeCallError::Internal("expected Temporal.PlainYearMonth".into()))
}

fn wrap_plain_year_month(
    pym: temporal_rs::PlainYearMonth,
    runtime: &mut crate::interpreter::RuntimeState,
) -> RegisterValue {
    let proto = runtime.intrinsics().temporal_plain_year_month_prototype();
    let handle = construct_temporal(TemporalPayload::PlainYearMonth(pym), proto, runtime);
    RegisterValue::from_object_handle(handle.0)
}

/// Extracts a PlainYearMonth from an argument — accepts objects or ISO strings.
fn to_plain_year_month(
    val: RegisterValue,
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<temporal_rs::PlainYearMonth, VmNativeCallError> {
    if let Some(handle) = val.as_object_handle().map(ObjectHandle)
        && let Ok(payload) = runtime.native_payload::<TemporalPayload>(handle)
        && let Some(pym) = payload.as_plain_year_month()
    {
        return Ok(pym.clone());
    }
    let s = to_string_arg(&[val], 0, runtime)?;
    temporal_rs::PlainYearMonth::from_utf8(s.as_bytes()).map_err(|e| temporal_err(e, runtime))
}

// ── Constructor ─────────────────────────────────────────────────────

/// §13.2.1 new Temporal.PlainYearMonth ( year, month [, calendar, referenceDay ] )
/// <https://tc39.es/proposal-temporal/#sec-temporal.plainyearmonth>
fn pym_constructor(
    _this: &RegisterValue,
    args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let year = to_integer_or_zero(args, 0, runtime)? as i32;
    let month = to_integer_or_zero(args, 1, runtime)? as u8;
    let ref_day = if args.len() > 3 {
        Some(to_integer_or_zero(args, 3, runtime)? as u8)
    } else {
        None
    };

    let pym = temporal_rs::PlainYearMonth::new_iso(year, month, ref_day)
        .map_err(|e| temporal_err(e, runtime))?;
    Ok(wrap_plain_year_month(pym, runtime))
}

// ── Static methods ──────────────────────────────────────────────────

/// §13.2.2.1 Temporal.PlainYearMonth.from ( item )
/// <https://tc39.es/proposal-temporal/#sec-temporal.plainyearmonth.from>
fn pym_from(
    _this: &RegisterValue,
    args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let val = args.first().copied().unwrap_or(RegisterValue::undefined());
    let pym = to_plain_year_month(val, runtime)?;
    Ok(wrap_plain_year_month(pym, runtime))
}

/// §13.2.2.2 Temporal.PlainYearMonth.compare ( one, two )
/// <https://tc39.es/proposal-temporal/#sec-temporal.plainyearmonth.compare>
fn pym_compare(
    _this: &RegisterValue,
    args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let one = args.first().copied().unwrap_or(RegisterValue::undefined());
    let two = args.get(1).copied().unwrap_or(RegisterValue::undefined());
    let a = to_plain_year_month(one, runtime)?;
    let b = to_plain_year_month(two, runtime)?;
    let result = match a.compare_iso(&b) {
        Ordering::Less => -1i32,
        Ordering::Equal => 0,
        Ordering::Greater => 1,
    };
    Ok(RegisterValue::from_i32(result))
}

// ── Getters ─────────────────────────────────────────────────────────

macro_rules! pym_getter_num {
    ($name:ident, $method:ident) => {
        fn $name(
            this: &RegisterValue,
            _args: &[RegisterValue],
            runtime: &mut crate::interpreter::RuntimeState,
        ) -> Result<RegisterValue, VmNativeCallError> {
            let pym = require_plain_year_month(this, runtime)?;
            Ok(RegisterValue::from_number(pym.$method() as f64))
        }
    };
}

/// §13.2.3.2 get Temporal.PlainYearMonth.prototype.calendarId
/// <https://tc39.es/proposal-temporal/#sec-get-temporal.plainyearmonth.prototype.calendarid>
fn pym_calendar_id(
    this: &RegisterValue,
    _args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let pym = require_plain_year_month(this, runtime)?;
    let id = pym.calendar().identifier();
    let handle = runtime.alloc_string(id.to_string());
    Ok(RegisterValue::from_object_handle(handle.0))
}

pym_getter_num!(pym_year, year);
pym_getter_num!(pym_month, month);

/// §13.2.3.5 get Temporal.PlainYearMonth.prototype.monthCode
/// <https://tc39.es/proposal-temporal/#sec-get-temporal.plainyearmonth.prototype.monthcode>
fn pym_month_code(
    this: &RegisterValue,
    _args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let pym = require_plain_year_month(this, runtime)?;
    let code = pym.month_code().as_str().to_string();
    let handle = runtime.alloc_string(code);
    Ok(RegisterValue::from_object_handle(handle.0))
}

pym_getter_num!(pym_days_in_month, days_in_month);
pym_getter_num!(pym_days_in_year, days_in_year);
pym_getter_num!(pym_months_in_year, months_in_year);

/// §13.2.3.9 get Temporal.PlainYearMonth.prototype.inLeapYear
/// <https://tc39.es/proposal-temporal/#sec-get-temporal.plainyearmonth.prototype.inleapyear>
fn pym_in_leap_year(
    this: &RegisterValue,
    _args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let pym = require_plain_year_month(this, runtime)?;
    Ok(RegisterValue::from_bool(pym.in_leap_year()))
}

// ── Prototype methods ───────────────────────────────────────────────

/// §13.2.3.11 Temporal.PlainYearMonth.prototype.add ( duration )
/// <https://tc39.es/proposal-temporal/#sec-temporal.plainyearmonth.prototype.add>
fn pym_add(
    this: &RegisterValue,
    args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let pym = require_plain_year_month(this, runtime)?;
    let dur_val = args.first().copied().unwrap_or(RegisterValue::undefined());
    let dur = to_duration(dur_val, runtime)?;
    let result = pym
        .add(&dur, temporal_rs::options::Overflow::default())
        .map_err(|e| temporal_err(e, runtime))?;
    Ok(wrap_plain_year_month(result, runtime))
}

/// §13.2.3.12 Temporal.PlainYearMonth.prototype.subtract ( duration )
/// <https://tc39.es/proposal-temporal/#sec-temporal.plainyearmonth.prototype.subtract>
fn pym_subtract(
    this: &RegisterValue,
    args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let pym = require_plain_year_month(this, runtime)?;
    let dur_val = args.first().copied().unwrap_or(RegisterValue::undefined());
    let dur = to_duration(dur_val, runtime)?;
    let result = pym
        .subtract(&dur, temporal_rs::options::Overflow::default())
        .map_err(|e| temporal_err(e, runtime))?;
    Ok(wrap_plain_year_month(result, runtime))
}

/// §13.2.3.15 Temporal.PlainYearMonth.prototype.equals ( other )
/// <https://tc39.es/proposal-temporal/#sec-temporal.plainyearmonth.prototype.equals>
fn pym_equals(
    this: &RegisterValue,
    args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let pym = require_plain_year_month(this, runtime)?;
    let other_val = args.first().copied().unwrap_or(RegisterValue::undefined());
    let other = to_plain_year_month(other_val, runtime)?;
    Ok(RegisterValue::from_bool(
        pym.compare_iso(&other) == Ordering::Equal,
    ))
}

/// §13.2.3.16 Temporal.PlainYearMonth.prototype.toString ( [ options ] )
/// <https://tc39.es/proposal-temporal/#sec-temporal.plainyearmonth.prototype.tostring>
fn pym_to_string(
    this: &RegisterValue,
    _args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    let pym = require_plain_year_month(this, runtime)?;
    let text = pym.to_ixdtf_string(temporal_rs::options::DisplayCalendar::Auto);
    let handle = runtime.alloc_string(text);
    Ok(RegisterValue::from_object_handle(handle.0))
}

/// §13.2.3.17 Temporal.PlainYearMonth.prototype.toJSON ( )
/// <https://tc39.es/proposal-temporal/#sec-temporal.plainyearmonth.prototype.tojson>
fn pym_to_json(
    this: &RegisterValue,
    args: &[RegisterValue],
    runtime: &mut crate::interpreter::RuntimeState,
) -> Result<RegisterValue, VmNativeCallError> {
    pym_to_string(this, args, runtime)
}
