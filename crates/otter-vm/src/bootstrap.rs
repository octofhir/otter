...
use crate::{
    ...
    descriptor_value, function_prototype, json, math,
...
pub(crate) fn native_new_target_prototype(
    ...
    let proto = if let Some(exec) = ctx.execution_context().cloned() {
        ...
        if let Some(class) = new_target.as_class_constructor() {
...
impl crate::intrinsic_install::BuiltinIntrinsic for IteratorIntrinsic {
    ...
    fn install(heap: &mut otter_gc::GcHeap, global: JsObject) -> Result<(), JsSurfaceError> {
        ...
        define_global_value(global, heap, Self::NAME, Value::native_function(ctor));
