...
pub struct Function {
    ...
    /// Used to decide when to deoptimize (invalidate JIT code).
    #[serde(skip)]
    pub bailout_count: AtomicU32,

    /// Whether this function has been deoptimized (JIT code invalidated).
    /// Once deoptimized, the function will never be re-queued for JIT compilation.
    #[serde(skip)]
...
impl Function {
    ...
    /// (i.e., the bailout count crossed the deoptimization threshold).
    #[inline]
    pub fn record_bailout(&self, deopt_threshold: u32) -> bool {
        ...
        if new_count >= deopt_threshold {
            // Mark as deoptimized (only succeeds once)
            self.is_deoptimized
                .compare_exchange(false, true, Ordering::Release, Ordering::Relaxed)
                .is_ok()
        } else {
