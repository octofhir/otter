...
- [x] Remove the measured duplicate instruction/operand decoding path: the VM
  ...
  GC differential 13/13. (boxed-double-property's `checksum + value * scale` is
  not yet fused — its two operators are separated by a property load, and the
  detector fuses only adjacent operations — so it is the next chain to reach.)
