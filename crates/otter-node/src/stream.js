...
//
// Every constructor here is a plain function, as in Node: callable without
// `new`, and usable as `Base.call(this, options)` by subclasses — the corpus
// leans on both forms (`http.Server(fn)`, `net.Socket.call(this)`).
...
Readable.prototype._destroy = function _destroy(err, cb) { cb(err); };
...
Readable.prototype.addListener = function addListener(ev, fn) {
...
Writable.prototype._destroy = function _destroy(err, cb) { cb(err); };
Writable.prototype._final = function _final(cb) { cb(); };

Writable.prototype.cork = function cork() { this._writableState.corked += 1; };
Writable.prototype.uncork = function uncork() {
  const state = this._writableState;
  if (state.corked) { state.corked -= 1; if (!state.corked) clearBuffer(this); }
};

Writable.prototype.setDefaultEncoding = function setDefaultEncoding(enc) {
...
PassThrough.prototype._transform = function _transform(chunk, _enc, cb) { cb(null, chunk); };
...
// Duplex mixes Writable's *methods* before the getters above exist, so copy
// the writable accessors onto it explicitly.
