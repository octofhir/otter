...
//! plus the proxy / property / call / coercion / instance-of / iterator / alloc
//! / install / closure / payload / ordinary-property / errors method clusters.
//!
//! TODO(Phase 2.2+): split this into a `runtime_state/` subdirectory with
//! per-cluster files (`errors.rs`, `realm.rs`, `property_names.rs`,
//! `accessors.rs`, `host.rs`, `proxy.rs`, `alloc.rs`, `install.rs`,
//! `closures.rs`, `payloads.rs`, `ordinary_property.rs`, `call.rs`,
//! `coercion.rs`, `instance_of.rs`, `iterators.rs`, `eval.rs`).
...
use super::number_conv::{canonical_string_exotic_index, parse_string_to_number, f64_to_int32, f64_to_uint32};
use super::{
...
impl RuntimeState {
    ...
    pub(super) fn delete_named_property(
        &mut self,
        target: ObjectHandle,
        property: PropertyNameId,
    ) -> Result<bool, InterpreterError> {
    ...
    pub(super) fn computed_property_name(
        &mut self,
        key: RegisterValue,
    ...
    pub(super) fn update_closure_function_name(
        &mut self,
    ...
    pub(super) fn is_primitive_property_base(&self, value: RegisterValue) -> Result<bool, ObjectError> {
    ...
    pub(super) fn js_abstract_relational_comparison(
        &mut self,
        lhs: RegisterValue,
        rhs: RegisterValue,
        left_first: bool,
    ...
    pub(super) fn alloc_reference_error(&mut self, message: &str) -> Result<ObjectHandle, InterpreterError> {
    ...
    pub(super) fn bigint_checked_div(
        &mut self,
        lhs: RegisterValue,
        rhs: RegisterValue,
    ) -> Result<RegisterValue, InterpreterError> {
    ...
    pub(super) fn bigint_checked_rem(
        &mut self,
        lhs: RegisterValue,
        rhs: RegisterValue,
    ) -> Result<RegisterValue, InterpreterError> {
    ...
    pub(super) fn js_typeof(&mut self, value: RegisterValue) -> Result<RegisterValue, InterpreterError> {
