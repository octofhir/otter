...
fn idx_operand_auto_widens_to_extra_wide() {
    ...
    b.emit(Opcode::LdaConstStr, &[Operand::Idx(70_000)]).unwrap();
    let bc = b.finish().unwrap();
...
fn imm_operand_signed_extra_wide() {
    let mut b = BytecodeBuilder::new();
    b.emit(Opcode::LdaSmi, &[Operand::Imm(-2_000_000_000)]).unwrap();
    let bc = b.finish().unwrap();
    let insns = decode_all(bc.bytes());
    assert_eq!(insns[0].width, OperandWidth::ExtraWide);
...
mod transpile_tests {
    ...
    use crate::bytecode::{transpile, Opcode, Operand};
    ...
    fn transpiles_load_return_pair() {
        ...
        assert_eq!(insns.len(), 4);
        assert_eq!(insns[0].opcode, Opcode::LdaSmi);
        assert_eq!(insns[0].operands, vec![Operand::Imm(42)]);
        assert_eq!(insns[1].opcode, Opcode::Star);
        assert_eq!(insns[1].operands, vec![Operand::Reg(0)]);
        assert_eq!(insns[2].opcode, Opcode::Ldar);
        assert_eq!(insns[2].operands, vec![Operand::Reg(0)]);
        assert_eq!(insns[3].opcode, Opcode::Return);
    }
    ...
    fn transpiles_add_as_ldar_add_star() {
        ...
        let insns = decode_all(v2.bytes());
        assert_eq!(insns.len(), 3);
        assert_eq!(insns[0].opcode, Opcode::Ldar);
        assert_eq!(insns[0].operands, vec![Operand::Reg(1)]);
        assert_eq!(insns[1].opcode, Opcode::Add);
        assert_eq!(insns[1].operands, vec![Operand::Reg(2)]);
        assert_eq!(insns[2].opcode, Opcode::Star);
        assert_eq!(insns[2].operands, vec![Operand::Reg(0)]);
    }
    ...
    fn transpiles_move_as_mov() {
        ...
        assert_eq!(insns[0].opcode, Opcode::Mov);
    ...
    fn transpiles_forward_jump_if_false() {
        ...
        assert!(matches!(insns[3].opcode, Opcode::JumpIfToBooleanFalse));
    ...
    fn transpiles_backward_jump_loop_header() {
        ...
        assert_eq!(insns[1].opcode, Opcode::Jump);
    ...
    fn transpiles_canonical_arith_loop_body_fragment() {
        ...
        assert_eq!(insns[1].opcode, Opcode::Add);
        assert_eq!(insns[2].opcode, Opcode::Star);
        assert_eq!(insns[3].opcode, Opcode::LdaSmi);
        assert_eq!(insns[4].opcode, Opcode::Star);
        assert_eq!(insns[5].opcode, Opcode::Ldar);
        assert_eq!(insns[6].opcode, Opcode::BitwiseOr);
        assert_eq!(insns[7].opcode, Opcode::Star);
        assert_eq!(insns[8].opcode, Opcode::Mov);
    }
    ...
    fn transpiles_new_object_and_array() {
        ...
        let insns = decode_all(v2.bytes());
        assert_eq!(insns.len(), 4);
        assert_eq!(insns[0].opcode, Opcode::CreateObject);
        assert_eq!(insns[1].opcode, Opcode::Star);
        assert_eq!(insns[2].opcode, Opcode::CreateArray);
        assert_eq!(insns[3].opcode, Opcode::Star);
    }
    ...
    fn transpiles_get_iterator_and_close() {
        ...
        // IteratorClose r1.
        assert_eq!(insns.len(), 5);
        assert_eq!(insns[0].opcode, Opcode::GetIterator);
        assert_eq!(insns[2].opcode, Opcode::IteratorNext);
        assert_eq!(insns[4].opcode, Opcode::IteratorClose);
    }
    ...
    fn transpiles_yield_and_await() {
        ...
        assert_eq!(insns[1].opcode, Opcode::Yield);
        assert_eq!(insns[4].opcode, Opcode::Await);
    }
    ...
    fn transpiles_private_field_ops() {
        ...
        let v2 = transpile(&v1).expect("transpile ok");
        let insns = decode_all(v2.bytes());
        assert!(insns.iter().any(|i| i.opcode == Opcode::DefinePrivateField));
        assert!(insns.iter().any(|i| i.opcode == Opcode::GetPrivateField));
        assert!(insns.iter().any(|i| i.opcode == Opcode::SetPrivateField));
    }
    ...
    fn transpiles_assert_not_hole() {
        ...
        assert_eq!(insns.len(), 2);
        assert_eq!(insns[0].opcode, Opcode::Ldar);
        assert_eq!(insns[1].opcode, Opcode::AssertNotHole);
    }
    ...
    fn call_direct_without_context_reports_missing() {
        ...
        assert!(matches!(
            err,
            crate::bytecode::TranspileError::MissingFunctionContext { .. }
        ));
    }
...
mod e2e_tests {
    use crate::bytecode::{Bytecode as V1Bytecode, BytecodeRegister, Instruction as V1Instruction, JumpOffset};
    use crate::bytecode::{execute, transpile, Frame};
...
fn canonical_example_s_eq_s_plus_i_bitor_zero() {
    ...
    assert_eq!(insns[0].opcode, Opcode::Ldar);
    assert_eq!(insns[1].opcode, Opcode::Add);
    assert_eq!(insns[2].opcode, Opcode::BitwiseOrSmi);
    assert_eq!(insns[3].opcode, Opcode::Star);
