...
pub(super) fn install_plain_year_month(
    ...
    > = Box::new(move |this, args, ncx| {
        ...
        // Validate and create via temporal_rs
        let pym = temporal_rs::PlainYearMonth::try_new_iso(year, month as u8, Some(ref_day as u8))
            .map_err(temporal_err)?;

        // Store as TemporalValue
        store_temporal_inner(&obj, TemporalValue::PlainYearMonth(pym));
        ...
        PropertyDescriptor::Accessor {
            get: Some(Value::native_function_with_proto(
                |this, _args, _ncx| {
                    ...
                    let pym = extract_plain_year_month(&obj)?;
        ...
        PropertyDescriptor::Accessor {
            get: Some(Value::native_function_with_proto(
                |this, _args, _ncx| {
                    let obj = this
                        .as_object()
                        .ok_or_else(|| VmError::type_error("monthCode getter"))?;
                    let pym = extract_plain_year_month(&obj)?;
        ...
        PropertyDescriptor::Accessor {
            get: Some(Value::native_function_with_proto(
                |this, _args, _ncx| {
                    let obj = this
                        .as_object()
                        .ok_or_else(|| VmError::type_error("calendarId getter"))?;
                    let pym = extract_plain_year_month(&obj)?;
        ...
        PropertyDescriptor::Accessor {
            get: Some(Value::native_function_with_proto(
                |this, _args, _ncx| {
                    let obj = this
                        .as_object()
                        .ok_or_else(|| VmError::type_error("daysInYear getter"))?;
                    let pym = extract_plain_year_month(&obj)?;
        ...
        PropertyDescriptor::Accessor {
            get: Some(Value::native_function_with_proto(
                |this, _args, _ncx| {
                    let obj = this
                        .as_object()
                        .ok_or_else(|| VmError::type_error("daysInMonth getter"))?;
                    let pym = extract_plain_year_month(&obj)?;
        ...
        PropertyDescriptor::Accessor {
            get: Some(Value::native_function_with_proto(
                |this, _args, _ncx| {
                    let obj = this
                        .as_object()
                        .ok_or_else(|| VmError::type_error("monthsInYear getter"))?;
                    let pym = extract_plain_year_month(&obj)?;
        ...
        PropertyDescriptor::Accessor {
            get: Some(Value::native_function_with_proto(
                |this, _args, _ncx| {
                    let obj = this
                        .as_object()
                        .ok_or_else(|| VmError::type_error("inLeapYear getter"))?;
                    let pym = extract_plain_year_month(&obj)?;
            ...
            PropertyDescriptor::Accessor {
                get: Some(Value::native_function_with_proto(
                    |this, _args, _ncx| {
                        let obj = this
                            .as_object()
                            .ok_or_else(|| VmError::type_error("era getter"))?;
                        // Validate it's actually a PlainYearMonth
                        let _pym = extract_plain_year_month(&obj)?;
                        Ok(Value::undefined())
                    },
                    mm.clone(),
                    fn_proto.clone(),
    ...
    let to_string_fn = Value::native_function_with_proto_named(
        |this, args, ncx| {
            ...
            let display_cal = if !options_val.is_undefined() {
            ...
            let s = pym.to_ixdtf_string(display_cal);
    ...
    let value_of_fn = Value::native_function_with_proto_named(
        |_this, _args, _ncx| {
            Err(VmError::type_error(
                "use compare() or toString() to compare Temporal.PlainYearMonth",
            ))
        },
        mm.clone(),
        fn_proto.clone(),
    ...
    let equals_fn = Value::native_function_with_proto_named(
        |this, args, ncx| {
            ...
            Ok(Value::boolean(
                this_pym.compare_iso(&other_pym) == std::cmp::Ordering::Equal,
            ))
        },
        mm.clone(),
        fn_proto.clone(),
    ...
    let with_fn = Value::native_function_with_proto_named(
        |this, args, ncx| {
            ...
            if let Some(arg_obj) = fields_val.as_object() {
                ...
                if tt.is_some() {
                    return Err(VmError::type_error(
                        "a Temporal object is not a valid argument to with()",
                    ));
                }
            }
            ...
            let month_val =
                ncx.get_property_of_value(&fields_val, &PropertyKey::string("month"))?;
    ...
    let to_plain_date_fn = Value::native_function_with_proto_named(
        |this, args, ncx| {
            let obj = this
                .as_object()
                .ok_or_else(|| VmError::type_error("toPlainDate"))?;
            let this_pym = extract_plain_year_month(&obj)?;
            let fields_val = args.first().cloned().unwrap_or(Value::undefined());
    ...
    let subtract_fn = Value::native_function_with_proto_named(
        |this, args, ncx| {
            let obj = this
                .as_object()
                .ok_or_else(|| VmError::type_error("subtract"))?;
            let this_pym = extract_plain_year_month(&obj)?;
            let dur_val = args.first().cloned().unwrap_or(Value::undefined());
            let dur = to_temporal_duration(ncx, &dur_val)?;
            let options_val = args.get(1).cloned().unwrap_or(Value::undefined());
    ...
    let until_fn = Value::native_function_with_proto_named(
        |this, args, ncx| {
            let obj = this
                .as_object()
                .ok_or_else(|| VmError::type_error("until"))?;
            let this_pym = extract_plain_year_month(&obj)?;
            let other_val = args.first().cloned().unwrap_or(Value::undefined());
            let other_pym = to_temporal_year_month(ncx, &other_val)?;
            let options_val = args.get(1).cloned().unwrap_or(Value::undefined());
    ...
    let since_fn = Value::native_function_with_proto_named(
        |this, args, ncx| {
            let obj = this
                .as_object()
                .ok_or_else(|| VmError::type_error("since"))?;
            let this_pym = extract_plain_year_month(&obj)?;
            let other_val = args.first().cloned().unwrap_or(Value::undefined());
            let other_pym = to_temporal_year_month(ncx, &other_val)?;
            let options_val = args.get(1).cloned().unwrap_or(Value::undefined());
    ...
    let get_iso_fields_fn = Value::native_function_with_proto_named(
        |this, _args, _ncx| {
            let obj = this
                .as_object()
                .ok_or_else(|| VmError::type_error("getISOFields"))?;
            // Validate it's a PlainYearMonth
            let _pym = extract_plain_year_month(&obj)?;
    ...
    proto.define_property(
        ...
        PropertyDescriptor::builtin_method(get_iso_fields_fn),
    );
    ...
    let from_fn = Value::native_function_with_proto_named(
        |_this, args, ncx| {
            ...
            if item.is_string() {
                ...
                return construct_plain_year_month_value(ncx, &pym);
            }
            ...
            {
                return Err(VmError::type_error(format!(
                    "cannot convert {} to a PlainYearMonth",
                    item.type_of()
                )));
            }
            ...
            if let Some(obj) = item.as_object() {
                ...
                // PlainDate -> extract year/month via TemporalValue
                if let Ok(pd) = extract_plain_date(&obj) {
                    if !options_val.is_undefined() {
                        parse_overflow_option(ncx, &options_val)?;
                    }
                    let pym = temporal_rs::PlainYearMonth::try_new_iso(pd.year(), pd.month(), None)
                        .map_err(temporal_err)?;
                    return construct_plain_year_month_value(ncx, &pym);
                }
            }
            ...
            if is_object_like {
                ...
                // Property bag calendar uses lenient validation (accepts ISO strings)
                let cal_val = ncx.get_property_of_value(&item, &PropertyKey::string("calendar"))?;
                if !cal_val.is_undefined() {
                    resolve_calendar_from_property(ncx, &cal_val)?;
                }
                ...
                // monthCode — read, ToPrimitive(string), RequireString
                let month_code_val =
                    ncx.get_property_of_value(&item, &PropertyKey::string("monthCode"))?;
                let month_code_str = if !month_code_val.is_undefined() {
                    let mc_js = to_primitive_require_string(ncx, &month_code_val)?;
                    let s = mc_js.as_str().to_string();
            ...
            Err(VmError::type_error(
                "Expected an object or string for PlainYearMonth.from",
            ))
        },
        mm.clone(),
        fn_proto.clone(),
...
fn to_temporal_year_month(
    ...
    if !is_object_like {
        return Err(VmError::type_error(
            "Expected an object or string for PlainYearMonth",
        ));
    }
...
/// Helper to construct a PlainYearMonth JS value from a temporal_rs::PlainYearMonth.
/// Calls the constructor with the appropriate arguments.
fn construct_plain_year_month_value(
    ncx: &mut NativeContext<'_>,
    ...
    ncx.call_function_construct(
        ...
        Value::undefined(),
        &[
            Value::int32(pym.year()),
            Value::int32(pym.month() as i32),
            Value::string(JsString::intern("iso8601")),
            Value::int32(pym.reference_day() as i32),
        ],
    )
}

/// Validate calendar arg for PlainYearMonth constructor — delegates to shared validator
fn validate_calendar_arg_for_pym(ncx: &mut NativeContext<'_>, cal: &Value) -> Result<(), VmError> {
    validate_calendar_arg_standalone(ncx, cal)?;
    Ok(())
}
...
fn validate_options_type(val: &Value) -> Result<(), VmError> {
    ...
    if val.is_string() {
        return Err(VmError::type_error(
            "string is not a valid options argument",
        ));
    }
