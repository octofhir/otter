...
(function installWebGlobals(global) {
  ...
  // Spec-side validation (exact DOMException names, the 65536-byte quota)
  // lives here; the CSPRNG and digest primitives are the hidden
  // `__otterCrypto*` natives installed from `crypto.rs`.
  {
    ...
    class SubtleCrypto {
      async digest(algorithm, data) {
        ...
        return global.__otterCryptoDigest(name, data);
      }
    }
    ...
    class Crypto {
      getRandomValues(array) {
        ...
        return global.__otterCryptoRandomFill(array);
      }

      randomUUID() {
        return global.__otterCryptoRandomUUID();
      }
