...
//! scoped installer fn that `HostedModule::new(<prefix>:<name>,
//! install_fn)` consumes, plus a `pub static
//! <UPPER>_HOSTED_MODULE: HostedModule` row callers drop into
//! their `HOSTED_MODULES` array.
//!
//! # Surface
//!
...
//!   -> Result<RuntimeLocal<'scope>, RuntimeNativeError>` — the
//!   scoped runtime installer.
...
pub(crate) fn expand(input: TokenStream) -> TokenStream {
    ...
    // Per-export installer fragments. Both allocate and publish the method in
    // the caller's one rooted scope:
    //   - capabilities = false → NativeScope::native_method
    //   - capabilities = true  → NativeScope::native_closure
    let export_installs = exports.iter().map(|ex| {
        ...
        if capabilities {
            ...
            quote! {
                {
                    let method = scope.native_method(#js_name, #length, #call)?;
                    scope.define(
                        namespace,
                        #js_name,
                        method,
    ...
    let capability_use = if capabilities {
        quote! {}
    } else {
        quote! {
            let _ = capabilities;
    ...
    let install_doc = format!(
        "Build the `{module_url}` hosted-module namespace in the supplied \
         [`RuntimeNativeScope`]. Generated by `lodge!`."
    );
