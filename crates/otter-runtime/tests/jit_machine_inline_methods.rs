//! Method-body SSA inlining with effect-once cold reentry.
//!
//! # Contents
...
"#,
    ...
    let fast = runtime
        .run_script(
            SourceInput::from_javascript(
                "var valid=0; for(var i=0;i<512;i++) if(caller(a,b,mark)===33) valid++; valid;",
            ),
...
JSON.stringify([value,reads,mark.before-beforeCount,mark.after-afterCount,
    ...
    stackText.indexOf('inline-property-warm.js:6:')>=0]);
"#,
            ),
    ...
    assert_eq!(
        ...
        "[21.75,1,1,1,true,true,true,true]"
    );
    let after = runtime.execution_stats();
    assert_eq!(
...
"#,
        ...
        .unwrap();
    assert_eq!(resumed.completion_string(), "[26.25,1,1,1,1,1]");
    assert!(
        resumed
            .jit_debug_report()
            .unwrap()
...
#[test]
fn method_inline_identity_misses_resume_lookup_and_call_once() {
    for (own, mutation, expected, lookup_count) in [
        (
            false,
            "proto.dot=function replacement(b){return this.x+b.x+100;};",
            "105.25",
            0,
        ),
        (
            true,
            "a.dot=function replacement(b){return this.x+b.x+100;};",
            "105.25",
            0,
        ),
        (
            false,
            "Object.defineProperty(proto,'dot',{get(){lookups++;return function replacement(b){return this.x+b.x+100;};}});",
            "105.25",
            1,
        ),
        (
            false,
            "Object.setPrototypeOf(a,{dot:function replacement(b){return this.x+b.x+100;}});",
            "105.25",
            0,
        ),
        (
            false,
            "proto.dot=(function(){return b=>this.x+b.x;}).call({x:40});",
            "44",
            0,
        ),
    ] {
        ...
        let warm = runtime
            .run_script(
                SourceInput::from_javascript(format!(
                    r#"
function dot(b) {{return this.x*b.x+this.y*b.y+this.z*b.z;}}
var proto={{dot:dot}}, a={{x:1.25,y:2,z:3}}, b={{x:4,y:5,z:6}};
...
JSON.stringify([result,lookups,mark.before-beforeCount,mark.after-afterCount]);
"#
                )),
                "method-identity-miss.js",
            )
            .unwrap();
        assert_eq!(
            result.completion_string(),
            format!("[{expected},{lookup_count},1,1]"),
            "{mutation}"
        );
        assert!(
            runtime.execution_stats().jit_optimized_deopts > before.jit_optimized_deopts,
            "identity miss must exit the compiled caller: {mutation}"
        );
    }
}
