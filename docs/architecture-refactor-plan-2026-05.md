...
### Task 1.1 — Replace `Value` Enum With 8-Byte Tagged Value
...
### Task 1.2 — Move Rc/Arc Value Payloads Into GC Bodies
...
2. Phase 1.1: `Value(u64)` hard cut-over.
3. Phase 1.2 and 1.3: GC payload migration and frame split. These can proceed
   in parallel only if file ownership is split cleanly.
4. Phase 2.1 and 2.2: bytecode v2 and single dispatch.
5. Phase 2.3, 2.4, 2.5: shortcut removal, PIC slots, native ABI.
6. Phase 3.1 and 3.2: bootstrap split and typed intrinsics. This can start
   after Phase 0 and run parallel to Phase 1 if it does not touch Value payloads.
7. Phase 4: macros after native ABI and `RealmIntrinsics`.
8. Phase 5: inspector after bytecode v2 and IC slots.
9. Phase 6: object/promise polish after Value unification; promise records can
