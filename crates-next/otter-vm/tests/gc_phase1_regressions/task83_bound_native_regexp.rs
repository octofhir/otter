...
fn bound_function_roots_target_this_and_args_when_rooted() {
    ...
    assert!(
        live_bytes(&mut interp, OBJECT_BODY_TYPE_TAG) >= object_baseline + 3,
        "rooted bound function must trace target, this, and bound args"
    );
...
fn native_function_captures_root_gc_values_when_rooted() {
    ...
    let Value::NativeFunction(native) = native else {
...
fn bound_native_and_regexp_unrooted_graphs_are_reclaimed() {
    ...
    assert_eq!(
        live_bytes(&mut interp, BOUND_FUNCTION_BODY_TYPE_TAG),
        bound_baseline
    );
    assert_eq!(
        live_bytes(&mut interp, NATIVE_FUNCTION_BODY_TYPE_TAG),
        native_baseline
    );
    assert_eq!(
        live_bytes(&mut interp, REGEXP_BODY_TYPE_TAG),
        regexp_baseline
    );
    assert_eq!(
        live_bytes(&mut interp, OBJECT_BODY_TYPE_TAG),
        object_baseline,
