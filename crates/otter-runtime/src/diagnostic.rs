...
mod tests {
    ...
    #[test]
    fn byte_offset_first_line_first_column_is_zero() {
        let src = "let x = 1;\nlet y = 2;\n";
        let loc = SourceLocation::new(1, 1);
        assert_eq!(byte_offset_for_location(src, loc), Some(0));
    }

    #[test]
    fn byte_offset_second_line() {
        let src = "abc\nthrow x;\n";
        // Line 2 col 1 should point at 't' in "throw" — that's byte 4.
        assert_eq!(
            byte_offset_for_location(src, SourceLocation::new(2, 1)),
            Some(4)
        );
    }

    #[test]
    fn byte_offset_emoji_line_uses_utf16() {
        // "a😀b" — 😀 is 1 char, 4 bytes, 2 UTF-16 code units.
        // Column 4 (1-based) means: skip a (1), skip 😀 (2), land at b.
        let src = "a😀b";
        let loc = SourceLocation::new(1, 4);
        // After 'a' (1 byte) + emoji (4 bytes) = byte 5.
        assert_eq!(byte_offset_for_location(src, loc), Some(5));
    }

    #[test]
    fn diagnostic_display_falls_back_to_rendered_stack() {
        let diag = JsRuntimeDiagnostic {
            rendered_stack: "TypeError: boom\n    at f (m.js:1:1)".to_string(),
            name: "TypeError".to_string(),
            message: "boom".to_string(),
            top_snippet: None,
            frames: Vec::new(),
        };
        assert_eq!(diag.to_string(), "TypeError: boom\n    at f (m.js:1:1)");
    }
}
