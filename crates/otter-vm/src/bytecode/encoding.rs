...
use super::opcodes::{Opcode, OperandShape};
...
struct LabelState {
    ...
    pending_immediates: Vec<PendingImmediate>,
}
...
/// An `Imm` operand whose value is the absolute PC of a `Label`.
/// Back-patched on `bind_label` once the label resolves.
...
impl BytecodeBuilder {
    ...
    pub fn bind_label(&mut self, label: Label) -> Result<(), EncodeError> {
        ...
        let pending_immediates = std::mem::take(&mut state.pending_immediates);
        for p in pending {
        ...
        let target_pc_i32 =
            i32::try_from(target_pc).map_err(|_| EncodeError::OperandOutOfRange {
                opcode: "LabelImm",
                position: 0,
                value: i64::from(target_pc),
            })?;
        for p in pending_immediates {
            Self::patch_signed(
                &mut self.bytes,
                p.operand_start as usize,
                p.operand_width,
    ...
    /// Emit an instruction whose sole operand is an `Imm` holding the
    /// absolute PC of `label`. Used by control-flow metadata opcodes that
    /// need to resume at a previously-generated bytecode address.
    pub fn emit_label_immediate(
        &mut self,
        opcode: Opcode,
        label: Label,
        ...
        assert_eq!(
            shape.arity(),
            1,
            "emit_label_immediate expects an instruction with exactly one Imm operand"
        );
        assert!(matches!(shape.operands()[0], OperandKind::Imm));
        ...
        if let Some(target_pc) = state.target_pc {
            let target = i32::try_from(target_pc).map_err(|_| EncodeError::OperandOutOfRange {
                opcode: opcode.name(),
                position: 0,
                value: i64::from(target_pc),
            })?;
            return self.emit(opcode, &[Operand::Imm(target)]);
        }

        let width = OperandWidth::ExtraWide;
        let pc = self.pc();
        self.emit_prefix(width);
        self.bytes.push(opcode.as_byte());
        let operand_start = self.pc();
